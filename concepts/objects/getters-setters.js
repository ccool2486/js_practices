...
(function() {
  function wrapValue(value) { // 回傳一個物件，裡面有getter跟setter方法
...
// 可以用class寫，但是context不一樣。詳見裡面的this
class ValueWrapper {
  constructor (value) {
    this.value = value
    console.log(this); // ValueWrapper
  }
  get getValue(){
    return this.value;
  }
}
var x = new ValueWrapper(5);
console.log(x.getValue);

// getter 和 setter 的传统使用方式
(function() {
  function wrapValue(_value) {
    return { // 回傳一個物件，裡面有getter跟setter方法
      get value() {
        return _value;
      },
      set value(newValue) {
